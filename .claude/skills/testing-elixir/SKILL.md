...
description: |
  ...
  controller/LiveView/channel tests, external API testing with Bypass and Req.Test,
  factory-first data setup patterns, and test organization.
...
| Factory-first data setup | [factory-patterns](references/factory-patterns.md) | insert() for relationships, StreamData, avoiding Repo.preload |
...
### Factory-First Data Setup

Use factories with `insert()` to build complete relationship structures. Let factories handle association complexity. Keep test setup declarative and focused on the behavior being tested.
...
- **Using Repo.preload for test setup**: Use factory associations to establish relationships, not Repo.preload.

## Reference File IDs

`core-exunit` . `assertions` . `sociable-testing` . `database-testing` . `phoenix-testing` . `external-api-testing` . `test-organization` . `factory-patterns`
