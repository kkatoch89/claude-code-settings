...
Slice a feature into thin vertical stories using Elephant Carpaccio methodology.

## Usage

`/writing-agile-stories slice` or `/writing-agile-stories slice [feature description]`
...
After presenting the backlog, confirm with the user:
- "Looks good" -- Proceed to writing stories for each slice
- "Adjust ordering" -- Reorder slices
- "Slices too thick -- split further"
- "Slices too thin -- combine some"

## Next Steps

After the slice backlog is confirmed:
- **Write stories** -- Use the `write` command to flesh out each slice into a BDD story with acceptance criteria
- **Decompose** -- Use the `decompose` command to break slices directly into implementation tasks

## Reference

See [slicing-carpaccio.md](../references/slicing-carpaccio.md) for the complete methodology reference.
