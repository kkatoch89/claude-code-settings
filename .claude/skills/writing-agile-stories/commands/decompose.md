...
Break a user story into small implementation tasks.

## Usage

`/writing-agile-stories decompose` or `/writing-agile-stories decompose [story or ticket reference]`

## When to Use

- A story is ready for sprint planning
- Decomposing user stories into implementation tasks
- Creating task lists from tickets
- User asks to "break down", "decompose", or "create tasks" for a story
...
| Spans multiple layers, multiple roles, no acceptance criteria | **Feature** | Suggest `slice` command first |
| References an epic or multiple stories | **Epic** | Suggest splitting into stories with `write` command first |
| Single technical concern, already small | **Task** | No decomposition needed -- inform the user |

If uncertain, ask whether to slice first or proceed directly.
...
- **Too vague**: If you can't identify concrete work, the story needs refinement -- flag this
- **Too large** (spans multiple epics): Suggest splitting into smaller stories first
- **Feature-sized input**: Suggest using `slice` to produce thin vertical slices first, then `write` for each slice, then return here
...
1. **Gather the Story**
   - If input is a ticket reference (e.g., "PROJ-123"), use available tools to fetch details
...
See [task-decomposition.md](../references/task-decomposition.md) for the full decomposition methodology.
