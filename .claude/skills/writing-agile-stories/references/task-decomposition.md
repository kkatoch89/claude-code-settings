...
- **Feature-sized input** (spans multiple layers, multiple user roles, or lacks acceptance criteria): Suggest slicing into thin vertical slices first, then writing stories for each slice, then return here for task decomposition
...
| Spans multiple layers, multiple roles, no acceptance criteria | **Feature** | Suggest slicing first |
| References an epic or multiple stories | **Epic** | Suggest splitting into stories first |
| Single technical concern, already small | **Task** | No decomposition needed |
