...
name: testing-react
description: |
  Expert guidance for writing tests for React applications using React Testing Library with either Vitest or Jest.
  ...
  Use when working with .test.ts/.test.tsx files, vitest.config.ts, jest.config.ts, React Testing Library,
...
Expert guidance for writing great tests for React applications using React Testing Library, with support for both Vitest and Jest as test runners.
...
| Query selection strategy | [query-priority](references/query-priority.md) | getByRole preferred, accessible queries, avoid CSS selectors |

## Test Runner Compatibility

This skill covers patterns for both Vitest and Jest. The core testing philosophy and React Testing Library patterns are identical across runners. Key differences:

| Feature | Vitest | Jest |
|---------|--------|------|
| Mock functions | `vi.fn()` | `jest.fn()` |
| Module mocks | `vi.mock()` | `jest.mock()` |
| Spy on methods | `vi.spyOn()` | `jest.spyOn()` |
| Fake timers | `vi.useFakeTimers()` | `jest.useFakeTimers()` |
| Restore mocks | `vi.restoreAllMocks()` | `jest.restoreAllMocks()` |
| Global stubs | `vi.stubGlobal()` | `global.fetch = jest.fn()` |
| Config file | `vitest.config.ts` | `jest.config.ts` |
| Setup file import | `@testing-library/jest-dom/vitest` | `@testing-library/jest-dom` |

The reference files use Vitest syntax (`vi.fn()`, `vi.mock()`) by default. When using Jest, substitute the equivalent `jest.*` calls.
...
- **Querying by index, CSS selectors, or class names**: These are fragile implementation details. Use accessible queries.

## Reference File IDs

`core-vitest` . `react-testing-library` . `user-event` . `sociable-testing` . `custom-hooks` . `component-patterns` . `assertions` . `query-priority`
