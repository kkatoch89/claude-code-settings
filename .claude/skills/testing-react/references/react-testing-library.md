...
RTL automatically calls `cleanup()` after each test when using Vitest or Jest. You should not need to call it manually.

If you see stale DOM between tests, ensure your config includes `globals: true` or that you are importing from `@testing-library/react` correctly.
