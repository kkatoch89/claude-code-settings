...
Custom matchers from `@testing-library/jest-dom` for asserting on DOM elements. These extend Vitest's (or Jest's) `expect` with DOM-specific assertions.
...
// Jest: src/test/setup.ts
import '@testing-library/jest-dom';
```

This registers all matchers with the test runner's `expect`. Import once in your setup file.
