# Safe Ecto Migrations Reference

Expert guidance for writing zero-downtime Ecto migrations that avoid table locks, blocked reads/writes, and other common pitfalls in Postgres and MySQL.

## Quick Start

```elixir
# Safe concurrent index -- no write blocking
...
> **NOTE**: This will not update the values of rows previously-set by the old default. This value has been materialized at the time of insert/update and therefore has no distinction between whether it was set by the column `DEFAULT` or set by the original operation.
>
> If you want to update the default of already-written rows, you must distinguish them somehow and modify them with a backfill.
...
This reference focuses exclusively on safe database migration patterns: avoiding table locks, preventing blocked reads/writes, and ensuring zero-downtime schema changes. It does not cover schema design, query optimization, or application-level Ecto patterns -- see [ecto](ecto.md) for those topics.
