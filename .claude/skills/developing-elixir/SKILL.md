...
description: |
  Comprehensive Elixir/Phoenix development expertise covering Phoenix controllers, routing, channels; LiveView components and real-time features; Ecto schemas, queries, migrations; OTP patterns (GenServer, Supervisor); functional domain modeling; GraphQL with Absinthe; Oban background jobs; quality pipeline; and ExUnit testing.

  Use when working with .ex/.exs files, mix projects, Phoenix applications, or when the user mentions Elixir, Phoenix, Ecto, LiveView, OTP, GenServer, Supervisor, Absinthe, Oban, or ExUnit. Also use for defmodule, mix.exs, umbrella apps, or iex sessions.
---
...
|------------|----------------|------------|
| Controllers, routing, plugs, channels | [phoenix](references/phoenix.md) | MVC patterns, authentication, WebSockets |
| LiveView components, forms | [liveview](references/liveview.md) | Real-time UI, hooks, streams |
| Schemas, queries, migrations | [ecto](references/ecto.md) | Associations, changesets, transactions |
| Safe migrations, schema changes | [safe-migrations](references/safe-migrations.md) | Concurrent indexes, zero-downtime DDL |
...
| Background jobs with Oban | [oban-patterns](references/oban-patterns.md) | Workers, retry strategies, queues, telemetry |
| Code quality checks | [quality-pipeline](references/quality-pipeline.md) | compile, format, credo, dialyzer |
...
-> Consult ecto.md for schema design, phoenix.md for controller
```
...
-> Consult oban-patterns.md for worker design, retry strategies, queue config
```

**Writing a database migration:**
```
User: "I need to add an index to the posts table"
-> Consult safe-migrations.md for concurrent index creation
```

**Running quality checks:**
```
User: "Make sure the code passes all checks"
-> Consult quality-pipeline.md for sequential validation steps
```
...
`phoenix` . `liveview` . `ecto` . `safe-migrations` . `otp-patterns` . `functional-modeling` . `graphql-absinthe` . `oban-patterns` . `quality-pipeline`
