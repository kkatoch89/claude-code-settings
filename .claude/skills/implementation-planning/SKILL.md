...
description: Creates detailed implementation plans through interactive research and iteration. Use when planning features, designing implementations, breaking down tasks into phases, or when the user asks "how should I implement X", "plan this feature", or wants a technical specification.
---
...
2. **Research** by exploring the codebase for relevant files and patterns
...
- User references a ticket or task to plan
- User asks "how should I implement X" or "plan this feature"
- A technical specification or phased breakdown is needed

## Core Principles

1. **Interactive**: Never dump a complete plan. Gather context, verify understanding, align on approach, then detail phases.
...
5. **Manual Verification**: Each phase ends with manual verification to ensure progress is on track. This is separate from TDD -- it validates the feature works as expected before proceeding.
...
1. **Verify corrections**: If user corrects you, research further. Don't just accept -- verify.

2. **Track progress**: Create a research checklist
...
When user approves structure, present the complete plan. Use the template in [references/plan-template.md](references/plan-template.md).
...
Expected behavior:
1. Read ticket
2. Research the codebase for relevant files and patterns
3. Present findings:
   ```
...
Context: Presented understanding that "the API uses REST endpoints"
User correction: "Actually, we're using GraphQL for this service"

Expected behavior:
1. **Don't just accept** -- verify by searching the codebase
...
- **researching-codebase**: For deeper codebase exploration before planning
