...
- Use `atomic-thought` to decompose scenarios systematically

**Phase 4 - Milestone Definition**: Scope first release, sequence features
- Use `tree-of-thoughts` for north star selection and prioritization

**Phase 5 - Review**: Verify coverage, check quality, save or proceed
- Use `self-consistency` to validate before finalizing
...
**User Impact = Scale x Value per User** (over your chosen time horizon)
...
Use structured thinking patterns at key decision points to produce visible, auditable reasoning.
...
### To Feature Slicing
`writing-prds` -> `writing-agile-stories` (slice command)
- Slice features from the use case compendium into thin vertical increments before writing stories

### To User Stories
`writing-prds` -> `writing-agile-stories` (write command)
...
GOOD: Every requirement references its source scenario
```
