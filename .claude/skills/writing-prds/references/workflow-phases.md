...
Present extraction summary, then confirm:
...
Present high-level requirements, then confirm:
...
After processing all scenarios, present the complete use case compendium as a **single unified table** (not separate tables per scenario), then confirm:
...
Present milestone definitions, then confirm:
...
- Options:
  ...
  - "Create implementation plan" -- Proceed to implementation planning
  - "Done" -- End workflow
