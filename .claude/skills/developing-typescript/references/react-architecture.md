...
Expert guidance for React component architecture, hooks patterns, state management, and composition strategies. Enhanced with BulletProof React principles.
...
These patterns from the BulletProof React methodology ensure scalable, maintainable applications.

### Colocation Strategy

Keep components, functions, styles, and state as close as possible to where they are used:

- Reduces redundant re-renders
- Improves code readability
- Minimizes cognitive load

When you see components, hooks, or utilities used in only one place but defined far away, move them closer to their usage.

### Nested Function Extraction

Avoid nested rendering functions within components. Extract UI units into separate components for better maintainability:
...
function UserDashboard() {
  ...
  return <div>{renderHeader()}<UserContent /></div>;
}
...
function UserDashboard() {
  return <div><DashboardHeader user={user} /><UserContent /></div>;
}
```

### Abstraction Layers for Third-Party Components

Wrap third-party components to adapt them to application needs. This makes future updates easier without cascading changes:

```typescript
// Bad: direct third-party usage scattered everywhere
import { DatePicker } from 'some-datepicker-lib';
// used directly in 20 components

// Good: application wrapper
import { DatePicker as LibDatePicker } from 'some-datepicker-lib';

export function DatePicker({ value, onChange, ...props }: AppDatePickerProps) {
  return <LibDatePicker selected={value} onDateChange={onChange} {...props} />;
}
```
...
### Prop Limit Guideline

When you see components with 7+ props or props passing through multiple layers unchanged, consider:
- Splitting the component into smaller, focused units
- Using composition patterns (children/slots) to prevent prop drilling
- Introducing context for genuinely shared state
...
- Apply linters and formatters for style enforcement
...
- Nested rendering functions inside components (extract them)
- Direct third-party component usage scattered across the codebase (wrap them)
...
Help developers build maintainable React applications through proper component architecture, effective state management, and thoughtful composition patterns following BulletProof React principles.
